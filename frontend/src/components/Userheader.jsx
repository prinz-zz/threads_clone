import { Avatar, Box, Button, Flex, Link, Menu, MenuButton, MenuItem, MenuList, Portal, Text, useToast, VStack } from "@chakra-ui/react"
import { BsInstagram } from "react-icons/bs"
import { CgMoreO } from "react-icons/Cg"
import { useRecoilValue } from "recoil"
import userScreenAtom from "../atom/userAtom"
import { Link as link } from 'react-router-dom'
import { useState } from "react"
import useShowToast from "../hooks/useShowToast"
import axios from 'axios'


export default function UserHeader({ user }) {

    const currentUser = useRecoilValue(userScreenAtom)// logged in user
    const [following, setFollowing] = useState(user.followers.includes(currentUser?._id))
    const showToast = useShowToast()
    const [updating, setUpdating] = useState(false)

    const handleFollowUnfollow = async () => {
        if(!currentUser){
            showToast('Error' , "Please login to follow", 'error')
            return
        }
        if(updating) return

        setUpdating(true)

        try {
            const res = await axios.post(`/api/users/follow/${user._id}`)
            const data = await res.data


            if (data.error) {
                showToast('Error', error.message, 'error')
                return
            }

            if (following) {
                showToast('Success', `Unfollowed ${user.username}`, 'success')
                user.followers.pop() //simulate in client side
            } else {
                showToast('Success', `Followed ${user.username}`, 'success')
                user.followers.push(currentUser?._id) //simulate in client side
            }

            setFollowing(!following)

            console.log(data);

        } catch (error) {
            // showToast('Error', error.message, 'error')
            showToast('Error', "You cannot follow/unfollow yourself", 'error')
        }finally{
            setUpdating(false)
        }
    }

    const toast = useToast({
        position: 'top',
        duration: 2000,
    })

    const copyUrl = () => {
        const currentUrl = window.location.href
        navigator.clipboard.writeText(currentUrl).then(() => {
            toast({ description: 'Url copied successfully' })
        }).catch(() => {
            toast({ description: 'Failed to copy' })
        })
    }


    return (
        <VStack gap={4} alignItems={"start"}>
            <Flex justifyContent={'space-between'} w={'full'}>
                <Box>
                    <Text fontSize={'2xl'} fontWeight={'bold'}>
                        {user.username}
                    </Text>
                    <Flex gap={2} alignItems={'center'}>
                        <Text fontSize={'sm'} fontWeight={"bold"}>{user.name}</Text>
                        <Text fontSize={'xs'} bg={'gray.dark'} color={'gray.light'} p={1} borderRadius={'full'}>threads.net</Text>
                    </Flex>
                </Box>

                <Box>
                    {user.profilePic && (
                        <Avatar
                            name={user.name}
                            src={user.profilePic}
                            size={{
                                base: "md",
                                md: "xl",
                            }}
                        />
                    )}
                    {!user.profilePic && (
                        <Avatar
                            name={user.name}
                            src='"https://unsplash.it/32/32"'
                            size={{
                                base: "md",
                                md: "xl",
                            }}
                        />
                    )}
                </Box>

            </Flex>


            <Text>{user.bio}</Text>
            {currentUser?._id === user._id &&
                <Link as={link} to='/update'>
                    <Button size={'sm'}>Update Profile</Button>
                </Link>
            }

            {currentUser?._id !== user._id &&
                <Button size={'sm'} onClick={handleFollowUnfollow} isLoading={updating}>{following ? 'Unfollow' : 'Follow'}</Button>
            }

            <Flex w={'full'} justifyContent={'space-between'}>
                <Flex gap={2} alignItems={'center'}>
                    <Text color={'gray.light'}>{user.followers.length}</Text>
                    <Box w={1} h={1} bg={'gray.light'} borderRadius={'full'}></Box>
                    <Link color={'gray.light'}>instagram.com</Link>
                </Flex>
                <Flex>
                    <Box className="icon-container">
                        <BsInstagram size={24} cursor={'pointer'} />
                    </Box>
                    <Box className="icon-container">
                        <Menu>
                            <MenuButton><CgMoreO size={24} cursor={'pointer'} /></MenuButton>
                            <Portal>
                                <MenuList bg={'gray.dark'}>
                                    <MenuItem bg={'gray.dark'} onClick={copyUrl}>Copy Link</MenuItem>
                                </MenuList>
                            </Portal>
                        </Menu>
                    </Box>
                </Flex>
            </Flex>

            <Flex w={'full'}>
                <Flex flex={1} borderBottom={'1.5px solid white'} justifyContent={'center'} pb={3} cursor={'pointer'}>
                    <Text fontWeight={'bold'}>Threads</Text>
                </Flex>
                <Flex flex={1} borderBottom={'1px solid gray'} color={'gray.light'} justifyContent={'center'} pb={3} cursor={'pointer'}>
                    <Text fontWeight={'bold'}>Replies</Text>
                </Flex>
            </Flex>

        </VStack>
    )
}